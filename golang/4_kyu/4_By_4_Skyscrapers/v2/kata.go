...
func SolvePuzzle(clues []int) [][]int {
	...
	for i := 0; !isFilled(town); i = (i+1)%(size*2) {
		...
		for _, p := range posses {
			if town[p[0]][p[1]] != 0 {
				hasNums = append(hasNums, nil)
			} else {
				hasNums = append(hasNums, getExistsNums(town, p[0], p[1]))
			}
		}
...
// getPosses returns all positions of a line in the town (if n < 4 it returns a vertical line, else horizontal one).
